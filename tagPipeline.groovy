        ...
        stage('Clone Repository') {
            steps {
                script {
                    ...
                    // Checkout the tag instead of a branch
                    sh """
                    git clone --branch ${env.BRANCH_NAME} --single-branch https://${GITHUB_TOKEN}@github.com/Gagan-R31/demo.git
                    """
                    COMMIT_SHA = sh(script: "git -C ${workspaceDir}/demo rev-parse --short HEAD", returnStdout: true).trim()
                    IMAGE_TAG = env.BRANCH_NAME.replaceFirst('refs/tags/', '')  // Use the tag name as the Docker tag
                }
            }
        }
